    ...
    if (empty($_SESSION['keranjang'])) {
        ...
        if (mysqli_query($koneksi, $insert_pelanggan)) {
            ...
            foreach ($_SESSION['keranjang'] as $item) {
                ...
                $jumlahBarang = (int) $_POST['berat'];
                $insert_penjualan = "INSERT INTO penjualan (idBarang, id, idPelanggan, jumlahBarang) 
...
<body>
    <nav class="nav text-white border-gray-200 dark:bg-gray-900 dark:border-gray-700 p-4">
        <div class="navi max-w-screen-xl flex items-center justify-between">
    ...
    <div class="main-content p-4 mt-5 mx-auto pl-0 max-w-screen-xl flex flex-col md:flex-row gap-4">
        <div class="product-container w-[400px] rounded h-auto">
            <?php while ($row = mysqli_fetch_array($result)): ?>
                <div class="product-item flex flex-col justify-center items-center w-full h-80 rounded-lg bg-gray-300 py-1 mt-2">
                    <div class="flex h-40 w-full justify-center items-center p-1 mt-4">
                        <img src="images/<?php echo $row['foto']; ?>"
                            style="max-width: 100%; max-height: 100%;border-radius: 10px;"
                            alt="<?php echo $row['namaBarang']; ?>">
                    </div>
                    ...
                    <strong>
                        <p class="text-sm">Rp<?= number_format($row['harga'], 0, ',', '.') ?>,00/(KG/Biji)</p>
                    </strong>
                    <div class="flex h-40 w-full justify-center items-center mb-4">
                    <form method="POST" action="#barang">
                        ...
                        <button type="submit" name="bayarkan"
                            class="block bg-green-300 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center border-black">Terima
                            Barang</button>
                    </form>
                    </div>
                </div>
    ...
    <footer class="footer">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
            ...
            <div class="flex space-x-4">
                <a href="https://www.instagram.com/manhagai_htb" target="_blank"
                    class="text-gray-300 hover:text-gray-400 transition duration-300">
                    <span class="">Instagram</span>
                    <img src="gambar/ig3.png" class="h-6 w-6 mx-10" fill="currentColor" viewBox="0 0 24 24"></img>
                </a>
                <a href="https://github.com/amanhutabarat17" target=""
                    class="text-white hover:text-gray-400 transition duration-300">
                    <span class="">amanhutabarat17</span>
                    <img src="gambar/github-mark-white.png" class="h-6 w-6 mx-10" fill="currentColor"
                ...
                <form method="POST" action="">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="border px-4 py-2 font-bold">
                                    Rp<?= number_format($_SESSION['harga1'], 0, ',', '.') ?>,00/KG/BIJI</td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr>
                    ...
                    <div class="flex justify-start items-center gap-3 mb-4">
                        ...
                        <input type="text" name="namaPembeli"
                            value="<?= isset($_POST['namaPembeli']) ? $_POST['namaPembeli'] : '' ?>" id="namaPembeli"
                            class="px-4 py-2 rounded-lg w-40">
                    </div>
                    <div class="flex justify-start items-center gap-3 mb-4">
                        <label for="berat">Berat/Jumlah Barang</label>
                        <input type="text" name="berat" id="berat" class="px-4 py-2 rounded-lg w-40"
                            value="<?= isset($_POST['berat']) ? $_POST['berat'] : '' ?>">
                        <button type="submit" name="hitung"
                            class="bg-blue-500 text-white px-4 py-2 rounded-lg">Hitung</button>
                    </div>
                    <div class="flex justify-end gap-2">
                        ...
                        <button name="batal" class="bg-blue-500 text-white px-4 py-2 rounded-lg">Batal</button>
                    </div>
                </form>
            </div>
        </div>
